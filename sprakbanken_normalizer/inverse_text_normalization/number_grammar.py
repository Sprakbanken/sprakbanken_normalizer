...
H3 = ((H1 | hundred) + CONJ + belowhundred).setParseAction(
    addfunc
)  # hundre og sekstiåtte
...
T2 = ((H3 | H1 | hundred) + WS + thousand).setParseAction(
    multfunc
)  # tre hundre og sekstiåtte tusen
T3 = ((T2 | T1 | thousand) + CONJ + belowhundred).setParseAction(
    addfunc
)  # femti tusen og sekstiåtte
T4 = ((T2 | T1) + WS + (H3 | H1)).setParseAction(
    addfuncnoconj
)  # tre hundre og femtiåtte tusen ni hundre
...
# Full grammars
numbergrammar = (
    WordStart() + (numpatterns) + WordEnd().setParseAction(lambda s, l, t: l)
)
numbergrammar_abovethirteen = (
    wstart
    + (T5 ^ H1 ^ H2 ^ H3 ^ H4 ^ T1 ^ T2 ^ T3 ^ T4 ^ numword_above_twelve)
...
if __name__ == "__main__":
    ...
    # tests
    print(
        numbergrammar.searchString(
            "vi har jo bare gjort dette cirka to og et halvt tusen ganger"
        )
    )
