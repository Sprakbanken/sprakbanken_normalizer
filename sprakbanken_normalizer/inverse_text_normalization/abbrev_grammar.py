...
# Rule compositions
simpleabbrev = oneOf(abbrevdict.keys()).setParseAction(
    lambda t: (t[0], abbrevdict[t[0]])
)  # Return abbreviations from abbrevdict
numpst = (numpatternszero + WS + pst).setParseAction(
    lambda t: (t[0][0] + " " + t[1][0], str(t[0][1]) + t[1][1])
)  # Convert "seksti prosent" to "60%" (note that correct orthography is "60 %")
numcomma = (numpatternszero + WS + comma + WS + numpatternszero).setParseAction(
    lambda t: (
        t[0][0] + " " + t[1][0] + " " + t[2][0],
        str(t[0][1]) + t[1][1] + str(t[2][1]),
    )
)  # Convert "seks komma fire" to "6,4"
numcommapst = (numcomma + WS + pst).setParseAction(
    lambda t: (t[0][0] + " " + t[1][0], t[0][1] + t[1][1])
)  # Convert "seks komma fire prosent" to "6,4%"
numandhalf = (numpatternszero + WS + andhalf).setParseAction(
    lambda t: (t[0][0] + " " + t[1][0], str(t[0][1]) + t[1][1])
)  # Convert "to og en halv" to "2,5"

# Complete grammar
abbrevgrammar = (
    wstart
    + (numandhalf ^ numcommapst ^ numpst ^ numcomma ^ simpleabbrev)
...
if __name__ == "__main__":
    ...
    print(
        abbrevgrammar.searchString(
            "blant annet satt hun i Statoils sitt styre og liknende store deler av én prosent"
        )
    )
    print(
        abbrevgrammar.searchString(
            "blant annet satt hun i Statoils sitt styre og liknende store deler av nittini prosent av tida"
        )
    )
    print(
        abbrevgrammar.searchString(
